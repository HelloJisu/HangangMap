...
function createCoffeeMarkers(bikeres) {
	...
	for (var i = 0; i < bikeres.length; i++) {
		...
		var markerImage = createMarkerImage(markerImageSrc, imageSize,
				imageOptions), marker = createMarker(bikeres[i].latitude,
				bikeres[i].longitude, markerImage);
...
function createStoreMarkers(roadres) {
	for (var i = 0; i < roadres.length; i++) {
		...
		// 마커이미지와 마커를 생성합니다
		var markerImage = createMarkerImage(markerImageSrc, imageSize,
				imageOptions), marker = createMarker(roadres[i].latitude,
				roadres[i].longitude, markerImage);
...
function createCarparkMarkers(parkingres) {
	for (var i = 0; i < parkingres.length; i++) {
		...
		// 마커이미지와 마커를 생성합니다
		var markerImage = createMarkerImage(markerImageSrc, imageSize,
				imageOptions), marker = createMarker(parkingres[i].latitude,
				parkingres[i].longitude, markerImage);
...
function createWaterMarkers(waterres) {
	for (var i = 0; i < waterres.length; i++) {
		...
		// 마커이미지와 마커를 생성합니다
		var markerImage = createMarkerImage(markerImageSrc, imageSize,
				imageOptions), marker = createMarker(waterres[i].latitude,
				waterres[i].longitude, markerImage);
...
function createFoodMarkers(foodres) {
	for (var i = 0; i < foodres.length; i++) {
		...
		// 마커이미지와 마커를 생성합니다
		var markerImage = createMarkerImage(markerImageSrc, imageSize,
				imageOptions), marker = createMarker(foodres[i].latitude,
				foodres[i].longitude, markerImage);
